...
$script:LogDir = Join-Path -Path (Split-Path $PSScriptRoot -Parent) -ChildPath 'LOG'
...
function Export-ReportData {
    ...
    switch ($Format) {
        ...
        'HTML' {
            ...
            $stylePath = Join-Path $PSScriptRoot 'Templates' 'style.css'
            $style = if (Test-Path $stylePath) { Get-Content $stylePath -Raw } else { @"
<style>
...
function Invoke-DirectoryPermissionAnalysis {
    ...
    $processFolder = {
        ...
        try {
            ...
            foreach ($gp in $groupPermissions) {
                ...
                if ($objectClass -eq 'group') {
                    ...
                    foreach ($m in $members) {
                        ...
                        if ($isDomain) {
                            $userObj = [PSCustomObject]@{ GroupName=$groupName; MUWID=$u.SamAccountName; Name= if ($u.Surname) { $u.Surname.ToUpper() } else { 'N/A' }; FullName= if ($u.GivenName) { $u.GivenName } else { $u.SamAccountName }; Aktiv=$u.Enabled }
                        } else {
                            $userObj = [PSCustomObject]@{ GroupName=$groupName; MUWID=$u.Name; Name=$u.Name.ToUpper(); FullName= if ($u.FullName) { $u.FullName } else { $u.Name }; Aktiv= if ($null -ne $u.Enabled) { $u.Enabled } else { $true } }
                        }
                    ...
                    if ($u) {
                        if ($isDomain) { $userObj = [PSCustomObject]@{ GroupName='DirectPermission'; MUWID=$u.SamAccountName; Name= if ($u.Surname) { $u.Surname.ToUpper() } else { 'N/A' }; FullName= if ($u.GivenName) { $u.GivenName } else { $u.SamAccountName }; Aktiv=$u.Enabled } }
                        else { $userObj = [PSCustomObject]@{ GroupName='DirectPermission'; MUWID=$u.Name; Name=$u.Name.ToUpper(); FullName= if ($u.FullName) { $u.FullName } else { $u.Name }; Aktiv= if ($null -ne $u.Enabled) { $u.Enabled } else { $true } } }
                        $userPermissions += $userObj
                    }
                }
            }
            ...
            $entries = @()
            foreach ($gp in $folderData.GroupPermissions) {
