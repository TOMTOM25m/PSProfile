...
$aclUser = "NT AUTHORITY\SYSTEM" # Language-neutral SID
...
Write-Host "=== CertWebService Installation Repair Tool v1.2 ===" -ForegroundColor Yellow
...
Write-Host "[OK] Administratorrechte bestaetigt." -ForegroundColor Green

# 2. Stoppe und loesche alte Scheduled Tasks
Write-Host "Stoppe und loesche existierende Scheduled Tasks..."
...
Write-Host "Bereinige alte URL ACLs fuer Ports: $($portsToClean -join ', ')..."
foreach ($port in $portsToClean) {
    $urlHttp = "http://+:$($port)/"
    $urlHttps = "https://+:$($port)/"
    try {
        # Versuche, sowohl http als auch https zur Sicherheit zu loeschen
...
# 5. Erstelle URL ACLs fuer SYSTEM neu
Write-Host "Erstelle neue URL ACLs fuer Benutzer '$aclUser'..."
foreach ($url in $urlsToReserve) {
    $command = "netsh http add urlacl url=$url user='$($aclUser)' listen=yes"
    Write-Host "  > $command"
    $output = Invoke-Expression $command
    if ($LASTEXITCODE -ne 0) {
        Write-Warning "Fehler beim Erstellen der URL ACL fuer '$url'. Ausgabe: $output"
    }
}
Write-Host "[OK] URL ACLs wurden erfolgreich erstellt." -ForegroundColor Green

# 6. Firewall-Regel erstellen/aktualisieren
Write-Host "Erstelle/Aktualisiere Firewall-Regel '$firewallRuleName' fuer Ports $($portsForFirewall -join ', ')..."
...
Write-Host "Test-NetConnection -ComputerName $computerName -Port 9080" -ForegroundColor White
Write-Host "Test-NetConnection -ComputerName $computerName -Port 9443" -ForegroundColor White
Write-Host ""
