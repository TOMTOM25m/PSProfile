...
# Samba credential management function # Import credential manager
...
if (Test-Path $CredManagerPath) {
    ...
    Write-Host "✅ Credential manager loaded" -ForegroundColor Green
} else {
    Write-Warning "FL-CredentialManager.psm1 not found - credential features disabled"
}
...
function Get-SambaCredential {
    ...
    try {
        ...
        if ($storedCred) {
            Write-Host "📁 Using stored server-specific Samba credentials for $Ser    foreach ($server in $servers) {
        $ipInfo = if ($server.IPAddress) { " [$($server.IPAddress)]" } else { "" }
        $conn = $server.Connectivity
        
        Write-Host "  - $($server.ServerName) -> $($server.FQDN)$ipInfo ($($server.ServerType))" -ForegroundColor Grayme" -ForegroundColor Green
        # Show Samba status with emoji
        $sambaEmoji = if ($conn.SambaSuccess) { "✅" } else { "❌" }
        $sambaStatus = if ($conn.SambaSuccess) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $sambaColor = if ($conn.SambaSuccess) { "Green" } else { "Red" }
        Write-Host "    💼 Samba (Port 445): $sambaEmoji $sambaStatus" -ForegroundColor $sambaColor
        ...
        $overallEmoji = if ($server.IsReachable) { "✅" } else { "❌" }
        $overallStatus = if ($server.IsReachable) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $overallColor = if ($server.IsReachable) { "Green" } else { "Red" }
        Write-Host "    🏁 Gesamt: $overallEmoji $overallStatus" -ForegroundColor $overallColor
        
        Write-Host "" # Empty line between serverseturn $storedCred
        }
    } catch { }
    ...
    Write-Host "⚠️ No Samba credentials found for $ServerName" -ForegroundColor Yellow
...
#region Helper Functionsss Deploy CertWebService from Excel - PowerShell 7.x Enhanced
.DESCRIPTION
    ...
    - Enhanced visuals with emojis and colors
.NOTES
    Encoding: UTF-8 with BOM (PowerShell 7.x enhanced)
    Version: 1.0.0
#>
...
if (Test-Path $CredManagerPath) {
    ...
    Write-Warning "⚠️ FL-CredentialManager.psm1 not found - credential features disabled"
}
...
function Resolve-ServerFqdn {
    ...
    foreach ($candidate in ($candidates | Sort-Object Priority)) {
        ...
        if ($connectivity.IsReachable) {
            Write-Result "Selected FQDN: $($candidate.FQDN) (Source: $($candidate.Source))" -Status "INFO"
    ...
    Write-Result "No reachable FQDN found, using: $($bestCandidate.FQDN) (Source: $($bestCandidate.Source))" -Status "INFO"
...
if ($DryRun) {
    ...
    foreach ($server in $servers) {
        ...
        Write-Host "  • $($server.ServerName) → $($server.FQDN)$ipInfo" -ForegroundColor Gray
        
        # Show ping status with emoji
        $pingEmoji = if ($conn.PingSuccess) { "✅" } else { "❌" }
        $pingStatus = if ($conn.PingSuccess) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $pingColor = if ($conn.PingSuccess) { "Green" } else { "Red" }
        Write-Host "    🏓 PING: $pingEmoji $pingStatus" -ForegroundColor $pingColor
        
        # Show individual port status with emojis
        $port80Emoji = if ($conn.Port80) { "✅" } else { "❌" }
        $port80Status = if ($conn.Port80) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $port80Color = if ($conn.Port80) { "Green" } else { "Red" }
        Write-Host "    🌐 Port 80: $port80Emoji $port80Status" -ForegroundColor $port80Color
        
        $port443Emoji = if ($conn.Port443) { "✅" } else { "❌" }
        $port443Status = if ($conn.Port443) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $port443Color = if ($conn.Port443) { "Green" } else { "Red" }
        Write-Host "    🔒 Port 443: $port443Emoji $port443Status" -ForegroundColor $port443Color
        
        $port9080Emoji = if ($conn.Port9080) { "✅" } else { "❌" }
        $port9080Status = if ($conn.Port9080) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $port9080Color = if ($conn.Port9080) { "Green" } else { "Red" }
        Write-Host "    ⚡ Port 9080: $port9080Emoji $port9080Status" -ForegroundColor $port9080Color
        
        $port9443Emoji = if ($conn.Port9443) { "✅" } else { "❌" }
        $port9443Status = if ($conn.Port9443) { "ERREICHBAR" } else { "NICHT ERREICHBAR" }
        $port9443Color = if ($conn.Port9443) { "Green" } else { "Red" }
        Write-Host "    🔐 Port 9443: $port9443Emoji $port9443Status" -ForegroundColor $port9443Color
...
foreach ($server in $servers) {
    ...
    Write-Host "🖥️  [$currentServer/$totalServers] $($server.ServerName) → $($server.FQDN)" -ForegroundColor Cyan
...
if ($results.Skipped.Count -gt 0) {
    Write-Host "⏭️  Skipped Servers:" -ForegroundColor Yellow
