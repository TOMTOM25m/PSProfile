...
<#
.SYNOPSIS
CertWebService - HTTP Web Service v2.7.0 - PowerShell 5.1 ASCII Edition
.DESCRIPTION
HTTP Web Service for Certificate Monitoring with Windows Certificate Store Data
Regelwerk v10.1.0 | PowerShell 5.1 Compatible | ASCII Only | Last Updated: 09.10.2025

FEATURES v2.7.0 (PowerShell 5.1 ASCII Edition):
- Pure PowerShell 5.1 compatibility
- ASCII-only character set (no UTF-8, no emojis)
...
- Clean ASCII Dashboard with maximum compatibility
...
# PowerShell 5.1 ASCII Configuration
...
function Write-ServiceLog {
    ...
    try {
        $logEntry | Out-File (Join-Path $logPath "CertWebService.log") -Append -Encoding ASCII
    } catch { }
...
function Get-HTMLDashboard {
    ...
    # Generate HTML with ASCII-only encoding
    $html = @"
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Certificate Surveillance Dashboard v2.7.0 - PowerShell 5.1 ASCII</title>
    <style>
        body { 
...
<body>
    <div class="header">
        <h1>Certificate Surveillance Dashboard <span class="version">v2.7.0</span><span class="ps51-badge">PowerShell 5.1</span><span class="ascii-badge">ASCII Only</span></h1>
        <div class="info">
            MedUni Wien | Windows Server Infrastructure | Port: 9080<br>
            Status: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') | Regelwerk v10.1.0 | ASCII Compatible | $totalCount Certificates
        </div>
    </div>
    ...
    <div class="container">
        ...
        <div class="table-container">
            <div class="table-header">Certificate Details - PowerShell 5.1 ASCII Compatible</div>
            <table>
                <thead>
                    <tr>
                        <th>Hostname</th>
...
"@
    ...
    $html += @"
        ...
        <div class="compatibility-info">
            <strong>PowerShell 5.1 ASCII Edition</strong> | 
            Maximum Compatibility Mode | 
            ASCII Character Set Only | 
            No UTF-8 Dependencies | 
...
# HTTP Listener Setup
Write-ServiceLog "Starting CertWebService v2.7.0 (PowerShell 5.1 ASCII) on port $Port"

try {
    $listener = New-Object System.Net.HttpListener
    $listener.Prefixes.Add("http://+:$Port/")
    $listener.Start()
    Write-ServiceLog "HTTP Listener started successfully on port $Port (PowerShell 5.1 ASCII)"
    ...
    while ($listener.IsListening) {
        try {
            ...
            # Set ASCII response headers (no UTF-8)
            $response.ContentType = "text/html; charset=ISO-8859-1"
            $response.ContentEncoding = [System.Text.Encoding]::ASCII
            $response.Headers.Add("Server", "CertWebService/2.7.0-PS51-ASCII")
            $response.Headers.Add("Cache-Control", "no-cache, no-store, must-revalidate")
            $response.Headers.Add("X-PowerShell-Version", "5.1")
            $response.Headers.Add("X-Character-Set", "ASCII")
            $response.Headers.Add("X-Compatibility-Mode", "Maximum")
            
            try {
                $html = Get-HTMLDashboard
                # Use ASCII encoding only
                $buffer = [System.Text.Encoding]::ASCII.GetBytes($html)
                $response.ContentLength64 = $buffer.Length
                $response.OutputStream.Write($buffer, 0, $buffer.Length)
                $response.StatusCode = 200
                Write-ServiceLog "Response sent successfully (200 OK, $($buffer.Length) bytes, ASCII)"
            } catch {
                $errorHtml = "<html><head><meta charset='ISO-8859-1'></head><body><h1>Error 500</h1><p>$($_.Exception.Message)</p></body></html>"
                $errorBuffer = [System.Text.Encoding]::ASCII.GetBytes($errorHtml)
