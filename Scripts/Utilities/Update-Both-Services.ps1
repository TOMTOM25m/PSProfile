...
Write-Host "  BEHEBT ALLE CERTRETRIEVALFEHLER" -ForegroundColor Green  
Write-Host "=========================================" -ForegroundColor Green
Write-Host ""

# 1. Update CertWebService
Write-Host "[1/4] Update CertWebService..." -ForegroundColor Cyan
...
if (Test-Path $moduleSource2) {
    ...
    Write-Host "      [FIX] (Domain)UVW und SUMME: nicht mehr als Server behandelt" -ForegroundColor Yellow
}
...
Write-Host "  ✓ CertWebService API Response jetzt kompatibel" -ForegroundColor White
Write-Host "    (Struktur mit 'total_count' hinzugefügt)" -ForegroundColor Gray
Write-Host "  ✓ _RetrievalMethod Property für alle Server" -ForegroundColor White  
Write-Host "    (Verhindert 'property not found' Fehler)" -ForegroundColor Gray
Write-Host "  ✓ Excel Block-Header werden korrekt gefiltert" -ForegroundColor White
Write-Host "    ('(Domain)UVW' und 'SUMME:' nicht mehr als Server)" -ForegroundColor Gray
Write-Host ""
