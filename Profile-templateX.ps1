...
.DESCRIPTION
    [DE] Diese Erweiterung für das PowerShell-Profil verbindet beim Start automatisch definierte 
         Netzlaufwerke. Die Liste der zu verbindenden Pfade wird dynamisch vom Reset-PowerShellProfiles.ps1 
         Skript befüllt. Die Verbindungslogik ist für PS 5.1 und PS 7+ optimiert und verwendet jeweils 
         die beste verfügbare Methode für die jeweilige PowerShell-Version.
...
# Die folgende Liste wird dynamisch vom Reset-PowerShellProfiles.ps1 Skript befüllt.
$networkPaths = #NETWORK_PATHS_PLACEHOLDER#

# Gemeinsame Logik für beide Versionen
    ...
    try {
        ...
        if ($_.Exception.GetBaseException().Message -like '*Zugriff wurde verweigert*') {
            try {
                ...
                $credential = Get-Credential -Message ("Anmeldeinformationen für '{0}' eingeben" -f $path)
                $driveLetter = Get-MUWNextFreeDriveLetter
...
# Implementierung für PowerShell 5.1
function Connect-MUWDrive_PS5 {
    param($path)
    Connect-MUWDriveInternal -path $path
}

# Implementierung für PowerShell 7+
